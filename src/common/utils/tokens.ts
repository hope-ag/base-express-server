import { REFRESH_SECRET_KEY, ACCESS_SECRET_KEY, JWT_TOKEN_ISSUER } from '@common/config';
...
export const createToken = (user: User, type: 'access' | 'refresh'): TokenData => {
  ...
  const secretKey: string = type === 'refresh' ? REFRESH_SECRET_KEY : ACCESS_SECRET_KEY;
...
export const extractTokenData = (token: string, type: 'access' | 'refresh'): DataStoredInToken => {
  const secretKey: string = type === 'refresh' ? REFRESH_SECRET_KEY : ACCESS_SECRET_KEY;
